...
for (let i = 0; i < 6200; i *= 2) {
  ...
  let inputChannels = random.shape[2]
  ...
  let modelPrep = () => {
    ...
    model.add(tf.layers.conv2d({ kernelSize: [1, 1], padding: 'valid', filters: 128, activation: 'linear', inputShape: [null, null, inputChannels] }));
    ...
    //model.add(tf.layers.leakyReLU({ alpha: 0.01 }));
    model.add(tf.layers.conv2d({ kernelSize: [1, 1], padding: 'valid', filters: 32, activation: 'linear', kernelRegularizer: 'l1l2', biasRegularizer: 'l1l2', }));
    model.add(tf.layers.leakyReLU({ alpha: 0.01 }));
    model.add(tf.layers.conv2d({ kernelSize: [1, 1], padding: 'valid', filters: 16, activation: 'linear', kernelRegularizer: 'l1l2', biasRegularizer: 'l1l2', }));
    model.add(tf.layers.batchNormalization({}));
    model.add(tf.layers.leakyReLU({ alpha: 0.01 }));

    model.add(tf.layers.conv2d({ kernelSize: [1, 1], padding: 'valid', filters: 8, activation: 'linear', kernelRegularizer: 'l1l2', biasRegularizer: 'l1l2', }));
    model.add(tf.layers.batchNormalization({}));
    model.add(tf.layers.leakyReLU({ alpha: 0.01 }));
