...
let modelPrep = () => {
    ...
    model.add(tf.layers.conv2d({ kernelSize:[1,1], padding:'valid', filters: 190, activation: 'linear', inputShape: [null, null, inputChannels] }));
    model.add(tf.layers.leakyReLU({ alpha: 0.01 }));
    model.add(tf.layers.conv2d({ kernelSize:[1,1], padding:'valid', filters: 120, activation: 'linear', }));
    model.add(tf.layers.leakyReLU({ alpha: 0.01 }));
    model.add(tf.layers.conv2d({ kernelSize:[1,1], padding:'valid', filters: 80, activation: 'linear', }));
    model.add(tf.layers.leakyReLU({ alpha: 0.01 }));
